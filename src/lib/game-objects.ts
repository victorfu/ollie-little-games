...
import { Platform, PlatformType, Collectible, CollectibleType } from './types'
...
export function getGapRange(heightProgress: number): { min: number; max: number } {
  if (heightProgress < GAME_CONFIG.DIFFICULTY.EASY.HEIGHT_THRESHOLD) {
    return { min: GAME_CONFIG.DIFFICULTY.EASY.GAP_Y.MIN, max: GAME_CONFIG.DIFFICULTY.EASY.GAP_Y.MAX }
  } else if (heightProgress < GAME_CONFIG.DIFFICULTY.MEDIUM.HEIGHT_THRESHOLD) {
    return { min: GAME_CONFIG.DIFFICULTY.MEDIUM.GAP_Y.MIN, max: GAME_CONFIG.DIFFICULTY.MEDIUM.GAP_Y.MAX }
  } else {
    return { min: GAME_CONFIG.DIFFICULTY.HARD.GAP_Y.MIN, max: GAME_CONFIG.DIFFICULTY.HARD.GAP_Y.MAX }
  }
}
